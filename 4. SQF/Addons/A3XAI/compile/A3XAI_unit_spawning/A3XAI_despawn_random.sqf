...
if (_isForceDespawn) then {
	...
	if (A3XAI_debugLevel > 0) then {diag_log format["A3XAI Debug: All units of random AI group spawned by trigger %1 have been killed. Starting force despawn in 30 seconds.",triggerText _trigger];};
	uiSleep 30;
} else {
	if (A3XAI_debugLevel > 0) then {diag_log format["A3XAI Debug: No players remain in %1. Deleting spawned AI in %2 seconds.",triggerText _trigger,A3XAI_randDespawnWait];};
	...
	uiSleep A3XAI_randDespawnWait;								//Wait some time before deleting units. (amount of time to allow units to exist when the trigger area has no players)
...
if (_canDespawn) then {
	...
	if (A3XAI_debugLevel > 0) then {diag_log format ["A3XAI Debug: A player has entered the trigger area at %1. Cancelling despawn script.",(triggerText _trigger)];}; //Exit script if trigger has been reactivated since A3XAI_randDespawnWait seconds has passed.
